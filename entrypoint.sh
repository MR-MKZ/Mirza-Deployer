...
# =========================================================
# HELPER: Telegram Retry Logic (Handles 429 Errors)
# =========================================================
telegram_retry() {
    local max_retries=5
    local attempt=1
    ...
    while [ $attempt -le $max_retries ]; do
        ...
        if echo "$response" | grep -q '"error_code":429'; then
            # Extract retry time from JSON (default to 5s if parsing fails)
            retry_after=$(echo "$response" | jq -r '.parameters.retry_after // 5')
            echo "⚠️  Rate limited. Sleeping for ${retry_after}s..."
            sleep "$retry_after"
        else
...
if [ -z "$(ls -A $TARGET_DIR 2>/dev/null)" ]; then
    ...
    if [ -z "$DOWNLOAD_URL" ] || [ "$DOWNLOAD_URL" == "null" ]; then
        echo "⚠️  API fetch failed or no release found. Falling back to main branch..."
...
# 2. Config & Webhook Generation
if [ ! -f "$CONFIG_FILE" ] || grep -q "{database_name}" "$CONFIG_FILE"; then
    echo "⚠️  Broken or missing config detected. Overwriting..."
...
// CONNECT TO 'db' SERVICE (Not localhost)
...
EOF
    ...
    if [ -f "$TARGET_DIR/table.php" ]; then
        php "$TARGET_DIR/table.php" || echo "❌ Warning: table.php execution failed."
    else
        echo "❌ Error: table.php not found at $TARGET_DIR/table.php"
    fi

    # --- Webhook Setup (With Retry) ---
    echo "🔗 Setting Telegram Webhook..."
    CLEAN_DOMAIN=$(echo "$DOMAIN_HOST" | sed 's~http[s]*://~~g')
    ...
    MESSAGE="✅ MirzaBot installed successfully via Docker."
